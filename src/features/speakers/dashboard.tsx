...
const filterSignedUp = (s: Session[], speakerId: string) =>
  s.filter(session =>
    session.speakers.some(speaker => speaker !== speakerId && Math.random() > 0.8)
  );
...
function AssignmentComponent({ session }: { session: Session }) {
  ...
  const Unassigned = () => (
    ...
    >
      <Button type="submit" class="text-md font-bold border-gray-700" title="Assign to me">
        Assign to me
      </Button>
    </form>
  );
...
function useAssignment(sessionId: string) {
  const [assignments, setAssignments] = makePersisted(
    ...
    { storage: localStorage, name: `assignments` }
  );
